 ...
 * 
 * This test function validates the core operations of a directed graph including:
 ...
 * 
 * The test creates a directed graph with 5 vertices (1->2, 1->3, 2->4, 3->5, 5->4),
 * performs various queries and modifications, and asserts expected results at each step.
 * 
 * @note This function has no parameters as it is a self-contained unit test.
 * @note This function does not return a value. Test failures are indicated by assertion failures.
 */
void test_directed_basic(){
    ...
    */
    ...
    g.removeEdge(gt::Graph<int>::Pointer(&data[1]), gt::Graph<int>::Pointer(&data[3]));
    g.removeEdge(gt::Graph<int>::Pointer(&data[0]), gt::Graph<int>::Pointer(&data[4]));
 ...
 * 
 * This test function validates the adjacency map operations of a directed graph including:
 ...
 * (1->2 weight:10, 1->3 weight:20, 2->4 weight:30, 3->5 weight:40, 5->4 weight:50),
 * then validates that both adjacency maps correctly represent the graph structure.
 * 
 * @note This function has no parameters as it is a self-contained unit test.
 * @note This function does not return a value. Test failures are indicated by assertion failures.
 */
void test_directed_Adjacency(){
    ...
    */
    ...
    std::unordered_map<gt::Graph<int>::Pointer, std::unordered_map<gt::Graph<int>::Pointer, int>> adjacyMap,reverseAdjacyMap;
...
void test_undirected_basic(){
    ...
    */
    ...
    g.removeEdge(gt::Graph<int>::Pointer(&data[1]), gt::Graph<int>::Pointer(&data[3]));
    g.removeEdge(gt::Graph<int>::Pointer(&data[5]), gt::Graph<int>::Pointer(&data[6]));
    assert(g.getIndegrees(&data[3])==1);
    assert(g.getOutdegrees(&data[1])==1);
